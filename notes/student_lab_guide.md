...
그래서 다음 실험에서는 `near_terminal_reward`를 추가합니다. 이것은 성공 조건을 쉽게 바꾸는 것이 아니라, 성공 반경 바로 바깥쪽의 아주 좁은 구간을 연결해 주는 보상입니다. 예를 들어 성공 기준이 `0.045m`라면, `0.050m` 안쪽까지 들어온 경우에만 작은 안내 보상을 줍니다.

이 보상은 단순히 가까워졌다고 항상 주지 않습니다. stage3가 준비되어 있고, 빨간 구체와 충돌하지 않으며, 빨간 구체 중심으로 곧게 들어가는 방향과 실제 push 진행이 함께 좋아야 커집니다. 즉, "근처에서 맴돌기"가 아니라 "마지막 정답 위치로 들어가기"를 구분하려는 시도입니다.

여기서 중요한 질문이 하나 있습니다. 보상과 벌점 기준이 이렇게 많아지는 것이 일반적일까요?

로봇팔 조작처럼 복잡한 문제에서는 여러 보상 항목을 쓰는 일이 흔합니다. 최종 성공만 보상하면 성공 경험이 너무 드물어서 학습이 시작되지 않을 수 있기 때문입니다. 그래서 접근, 정렬, 접촉, 진입, 충돌 회피처럼 중간 목표를 나누어 보상을 줍니다.

하지만 많다고 무조건 좋은 것은 아닙니다. 보상이 서로 반대 방향을 가리키면 정책이 흔들립니다. 중간 보상이 너무 크면 로봇팔이 최종 성공을 하지 않고 중간 상태에 머물 수도 있습니다. 그래서 이번 프로젝트에서는 새 보상을 넣을 때마다 "어떤 문제를 해결하기 위한 것인지", "어떤 지표가 좋아져야 하는지", "어떤 부작용을 봐야 하는지"를 함께 기록합니다.

이번 near-terminal 실험에서 볼 지표는 `success_rate`, `stage3_touch_ready_rate`, `stage4_center_ready_rate`, `mean_near_terminal_reward`, `mean_target_contact_penalty`입니다. 성공률이 오르더라도 충돌 벌점이 커지면 좋은 학습으로 보지 않습니다.

near-terminal 실험 결과, best checkpoint는 `model_1600.pt`였습니다. 주요 지표는 `success_rate=0.01416015625`, `stage3_touch_ready_rate=0.370849609375`, `mean_near_terminal_reward=0.034814152866601944`, `mean_target_contact_penalty=0.0`입니다.

이 결과는 조금 애매하지만 배울 점이 많습니다. 새 보상은 정상적으로 작동했고 충돌도 늘지 않았습니다. 하지만 선택된 checkpoint가 이번 run의 시작점에 가까웠기 때문에, 추가 학습이 최종 성공률을 더 끌어올렸다고 말하기는 어렵습니다. 학습 중간에는 stage3가 좋아졌지만 최종 성공은 오히려 낮아지는 구간도 있었습니다.

그래서 다음 아이디어는 stage latch입니다. stage latch는 한 단계를 통과하면 그 상태를 "확보했다"고 기록하고, 그 다음 단계 보상을 열어 주는 방식입니다. 예를 들어 로봇팔이 파란 구체 위치와 정렬을 잘 맞추면 stage3를 확보한 것으로 보고, 그 이후에는 빨간 구체 중심으로 들어가는 stage4를 더 강하게 학습합니다.

여기에 progressive stage weighting을 붙일 수 있습니다. 뒤 stage에서 성공에 가까워질수록 앞 stage를 잘 유지한 점수의 가치를 조금씩 높이는 것입니다. 학생들에게는 "게임에서 다음 문을 열려면 이전 열쇠를 들고 있어야 하고, 마지막 문에 가까워질수록 그 열쇠의 가치가 더 커진다"고 설명할 수 있습니다.

단, 이 방법도 조심해야 합니다. 앞 stage 점수가 너무 크면 로봇팔이 마지막으로 들어가지 않고 앞 stage에서 머무를 수 있습니다. 그래서 다음 실험에서는 latch를 먼저 로그로 분리하고, 가중치는 작게 시작하는 것이 좋습니다.
